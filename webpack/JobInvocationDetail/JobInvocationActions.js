...
export const enableRecurringLogics = (
  recurrenceId,
  enabled,
  jobId
) => dispatch => {
  const successToast = () =>
    enabled
      ? sprintf(__('Recurring logics %s disabled successfully.'), recurrenceId)
      : sprintf(__('Recurring logics %s enabled successfully.'), recurrenceId);
  const errorToast = response =>
    enabled
      ? sprintf(
          __(`Could not disable recurring logics %s: ${response}`),
          recurrenceId
        )
      : sprintf(
          __(`Could not enable recurring logics %s: ${response}`),
          recurrenceId
        );
  const url = enabled
    ? `/foreman_tasks/api/recurring_logics/${recurrenceId}`
    : `/foreman_tasks/api/recurring_logics/${recurrenceId}`;
  dispatch(
    APIActions.put({
      url,
      key: CHANGE_ENABLED_RECURRING_LOGICS,
      params: { recurring_logics: { enabled: !enabled } },
      successToast,
      errorToast: ({ response }) =>
        errorToast(
          // eslint-disable-next-line camelcase
...
export const cancelRecurringLogics = (recurrenceId, jobId) => dispatch => {
  const successToast = () =>
    sprintf(__('Recurring logics %s cancelled successfully.'), recurrenceId);
  const errorToast = response =>
    sprintf(
      __(`Could not cancel recurring logics %s: ${response}`),
      recurrenceId
    );
