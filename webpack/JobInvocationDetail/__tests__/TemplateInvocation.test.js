...
describe('TemplateInvocation', () => {
  ...
  test('filtering toggles', () => {
    ...
    expect(
      screen.getAllByText('No output for the selected filters')
    ).toHaveLength(1);
    ...
    expect(screen.getAllByText('Exit status: 1')).toHaveLength(1);
    expect(
      screen.queryAllByText('StandardError: Job execution failed')
    ).toHaveLength(0);
    ...
    expect(screen.getAllByText('Exit status: 1')).toHaveLength(1);
    expect(
      screen.getAllByText('StandardError: Job execution failed')
    ).toHaveLength(1);
  });
  test('Preview Template button', () => {
    render(
      <Provider store={store}>
        <TemplateInvocation
          hostID={1}
    ...
    expect(screen.queryAllByText('User input')).toHaveLength(0);
    expect(screen.queryAllByText('command')).toHaveLength(0);
    expect(
      screen.queryAllByText(/echo -e "\\e\[31mThis is red text\\e\[0m"/)
    ).toHaveLength(0);
    act(() => {
    ...
    expect(
      screen.getAllByText(/echo -e "\\e\[31mThis is red text\\e\[0m"/)
    ).toHaveLength(1);
  });
