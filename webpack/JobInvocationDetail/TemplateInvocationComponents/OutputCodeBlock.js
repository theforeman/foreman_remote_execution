...
export const OutputCodeBlock = ({ code, showOutputType, scrollElement }) => {
  ...
  const scrollElementSeleceted = useCallback(
    () => document.querySelector(scrollElement),
    [scrollElement]
  );
  const onClickScrollToTop = useCallback(() => {
    if (scrollElementSeleceted()) scrollElementSeleceted().scrollTo(0, 0);
  }, [scrollElementSeleceted]);

  const onClickScrollToBottom = useCallback(() => {
    if (scrollElementSeleceted())
      scrollElementSeleceted().scrollTo(
        0,
        scrollElementSeleceted().scrollHeight
      );
  }, [scrollElementSeleceted]);

  const [isScrolledBottom, setIsScrolledBottom] = useState(true);
  useEffect(() => {
    const element = scrollElementSeleceted();
    const onScroll = () => {
      if (
        Math.abs(
