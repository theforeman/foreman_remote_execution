  ...
  module RemoteExecution
    class RunHostJob < Actions::EntryAction
      ...
      def verify_permissions(host, template_invocation)
        ...
        infra_facet = host.infrastructure
        if (infra_facet&.foreman_uuid || infra_facet&.smart_proxy_uuid) && !User.current.can?(:execute_jobs_on_infrastructure_hosts)
          raise _('User can not execute job on an infrastructure host %s') % host.name
        end
